...
@cache
def count_arrangements(s, brokens, broken_ctr=0):
    def count_helper(s, brokens, broken_ctr):
        ...
        if s[0] == '.':
            if not brokens:
                count_helper(s[1:].strip('.'), brokens, broken_ctr)
            elif broken_ctr == brokens[0]:
                count_helper(s[1:].strip('.'), brokens[1:], 0)
            else:
                if broken_ctr == 0:
                    count_helper(s[1:].strip('.'), brokens, broken_ctr)
                else:
            ...
            if brokens[0] == broken_ctr:
                ...
                count_helper(s[1:], brokens, broken_ctr + 1)
        elif s[0] == '?':
            count_helper(s.replace('?', '#', 1), brokens, broken_ctr)
            count_helper(s.replace('?', '.', 1), brokens, broken_ctr)

    print(s, brokens, broken_ctr)
    count = 0
    # set1 = set()
    count_helper(s, brokens, broken_ctr)
    return count
